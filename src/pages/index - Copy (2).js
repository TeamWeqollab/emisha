                  ...
                  <li className="list-inline-item">
                    <div className="resource-home">
                      <div className="imageContainer-latesResourece position-relative">
                        <Image src="/images/img-whitePaper-2.jpg" alt="Emisha" width={480} height={360} className="img-fluid mask2" />
                      </div>
                       <div className="resource-homeDetail">
                          <span className="badge resourceTag">CASE STUDY</span>
                          <h3 className="resourceTitle">Company A: Master Data Management</h3>
                        </div>
                    </div>
                  </li>
                  <li className="list-inline-item">
                   ...
                   <li className="list-inline-item">
                    <div className="resource-home">
                      ...
                      <div className="resource-homeDetail">
                        <span className="badge resourceTag">HOW-TO</span>
                        <h3 className="resourceTitle">Using Data to Stay Ahead of the Curve</h3>
                      </div>
                    </div>
                  </li>
                </ul>
                {/* <div className="imageContainer-latestNews position-relative">
                    <Image src="/images/img-whitePaper-2.jpg" alt="Emisha" width={480} height={360} className="img-fluid mask2 mb-3" />
                </div> */}
                  ...
                  <div key={item.id || idx} className="col-md-4">
                    <Link href={`/news/${item.Slug || item.slug || ''}`}>
                      <div className="card latestNewsCard">
                        ...
                        <div className="card-body">
                          <h5 className="card-title">{item.Title || item.name || 'Untitled'}</h5>
                          <p className="card-date">{(item.Category && (item.Category.Name || item.Category.name)) || 'News'}   -    {formatDate(item.PublishDate)}</p>
                          <p className="card-text">{item.Summary || ''}</p>
                        </div>
                      </div>
                    </Link>
...
export async function getServerSideProps() {
  const base = process.env.NEXT_PUBLIC_API_URL || process.env.NEXT_PUBLIC_STRAPI_URL || 'http://localhost:1337';

  try {
    const res = await fetch(`${base}/api/homepage-setting?populate=*`);
    if (!res.ok) {
      console.error('Failed to fetch homepage-setting:', res.status, res.statusText);
      return { props: { homepageNews: [], homepageResources: [] } };
    }
    ...
    const normalizeArticle = (raw, idx) => {
      const node = raw?.attributes || raw || {};
      const nested = node.news_article || node.newsArticle || node.article || null;
      const source = nested ? (nested?.data?.attributes || nested?.attributes || nested?.data || nested) : node;
      return {
        id: raw?.id || source?.id || idx,
        Title: getValue(source, 'Title', 'title', 'Name', 'name') || '',
        Slug: getValue(source, 'Slug', 'slug') || '',
        Summary: getValue(source, 'Summary', 'summary', 'Description', 'description') || '',
        PublishDate: getValue(source, 'PublishDate', 'publishDate', 'publishedAt', 'createdAt') || '',
        Banner: getValue(source, 'Banner', 'banner', 'Image', 'image') || null,
        Category: getValue(source, 'Category', 'category', 'news_category') || null
      };
    };

    const normalizeResource = (raw, idx) => {
      const node = raw?.attributes || raw || {};
      const nested = node.resource || node.Resource || null;
      const source = nested ? (nested?.data?.attributes || nested?.attributes || nested?.data || nested) : node;
      return {
        id: raw?.id || source?.id || idx,
        Title: getValue(source, 'Title', 'title', 'Name', 'name') || '',
        Slug: getValue(source, 'Slug', 'slug') || '',
        Summary: getValue(source, 'Summary', 'summary', 'Description', 'description') || '',
        Image: getValue(source, 'Image', 'image', 'Banner') || null,
        ResourceType: getValue(source, 'ResourceType', 'resourceType', 'Type', 'type') || null
      };
    };

    const rawNews = toArray(root?.homepage_news || root?.homepageNews || root?.homepage_news_list);
    const rawResources = toArray(root?.homepage_resources || root?.homepageResources || root?.homepage_resources_list);

    const homepageNews = rawNews.map((r, i) => normalizeArticle(r, i));
    const homepageResources = rawResources.map((r, i) => normalizeResource(r, i));



    if (process.env.NODE_ENV === 'development') {
      try { console.log('getServerSideProps: raw homepage root sample', JSON.stringify(root).substring(0,2000)); } catch (e) {}
      console.log('getServerSideProps: built homepageNews count', homepageNews.length, 'homepageResources', homepageResources.length);
      if (homepageNews[0]) console.log('getServerSideProps sample homepageNews', homepageNews[0]);
    }

    return { props: { homepageNews, homepageResources } };
  } catch (error) {
    console.error('Error in getServerSideProps fetching homepage setting:', error);
    return { props: { homepageNews: [], homepageResources: [] } };
  }
}
